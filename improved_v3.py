...
img_back_smoothed = median_filter(img_back, size=3)
...
# Aplicar o filtro Wiener
img_back_wiener = wiener(img_back_final, (5, 5))

# Aplicar a Transformada Wavelet para denoising
img_back_wavelet = denoise_wavelet(img_back_wiener, rescale_sigma=True)

# Calcular o SSIM entre a imagem original e a filtrada
img_original = cv2.imread('folhas1.jpg', 0)
ssim_value = ssim(img_original, img_back_wavelet, data_range=img_back_wavelet.max()-img_back_wavelet.min())
